...
public class SplashScreen11 extends Canvas implements Runnable {
     ...
     */
    public SplashScreen11(Midlet projectMIDlet){
        this.projectMIDlet = projectMIDlet;
        try{
        mImage = Image.createImage("GoVoyage/Splashes/splash11.png");
    ...
    public void dismiss() {
        if (isShown())
            Display.getDisplay(projectMIDlet).setCurrent(new SplashScreen12(projectMIDlet));
    ...
    public void run() {
        try {
            Thread.sleep(250);//set for 3 seconds
        }

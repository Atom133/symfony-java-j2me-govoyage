 ...
 * @author sawsse
 */
public class SplashScreen_Dest extends Canvas implements Runnable {
     ...
     */
    public SplashScreen_Dest(Midlet projectMIDlet){
        this.projectMIDlet = projectMIDlet;
        try{
        mImage = Image.createImage("GoVoyage/Splashes/splash_dest.png");
    ...
    public void dismiss() {
        if (isShown())
           // Display.getDisplay(projectMIDlet).setCurrent(new login());
            Display.getDisplay(projectMIDlet).setCurrent(new DestinationMenu());

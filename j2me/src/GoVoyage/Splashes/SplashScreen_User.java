 ...
 * @author sawsse
 */
public class SplashScreen_User extends Canvas implements Runnable {
     ...
     */
    public SplashScreen_User(Midlet projectMIDlet){
        this.projectMIDlet = projectMIDlet;
        try{
        mImage = Image.createImage("GoVoyage/Splashes/splash_users.png");
    ...
    public void dismiss() {
        if (isShown())
           // Display.getDisplay(projectMIDlet).setCurrent(new login());
            Display.getDisplay(projectMIDlet).setCurrent(new PersonneList());
